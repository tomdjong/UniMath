...
Context (fibdeceq : ∏ (i : I), isdeceq (∑ (a : A), t a = i)).

Fixpoint indexedWtype'_code {i : I} (u v : indexedWtype' t s i) : UU :=
  match u with
  | indexedsup' _ _ _ a p f =>
    match v with
    | indexedsup' _ _ i a' p' f' =>
      match (fibdeceq i (tpair (λ (a : A), t a = i) a p) (tpair _ a' p')) with
      | inr _ => empty
      | inl e => ∏ (b : B a), @indexedWtype'_code
                                (s (a,,b))
                                (f b)
                                (transportb (indexedWtype' t s)
                                            (indexedWtype'_s_transport (pr1 e) b)
                                            (f' (transportf B (pr1 e) b)))
      end
    end
  end.
