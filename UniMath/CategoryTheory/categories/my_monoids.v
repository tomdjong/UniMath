...
Section char_monoid_iso.
  ...
  Lemma monoidiso_is_monoid_cat_z_iso {X Y : ob monoid_category} : monoidiso X Y -> z_iso X Y.
  Proof.
    intro f.
  ...
  Lemma monoid_cat_z_iso_isweq {X Y : ob monoid_category} (f : X --> Y) : is_z_isomorphism f -> isweq (pr1 f).
  Proof.
  ...
  Lemma monoid_cat_z_iso_is_monoidiso {X Y : ob monoid_category} : z_iso X Y -> monoidiso X Y.
  Proof.
    intro f.
    use monoidisopair.
    - use weqpair.
  ...
  Lemma monoidiso_is_monoid_cat_z_iso_equiv (X Y : ob monoid_category) : monoidiso X Y ≃ z_iso X Y.
  Proof.
    use weqpair.
...
Section def_monoid_univalent_category.
  ...
  Proof.
    ...
    - use mk_is_univalent.
      + intros X Y.
        ...
        * X = Y ≃ monoidiso X Y (this monoid_univalence from Monoids_and_Groups)
        *       ≃ z_iso X Y (proven above)
        *       ≃ iso X Y (this is z_isomorphism_iso_equiv from Categories) **)
...
End def_monoid_univalent_category.